...
# Skal have: tcwv, tcw, u10,v10, t2m, sst, 
...
for x in range(len(dec72)):
    ...
    for i in range(len(dec72[x].TBNEMS)):
        for j in range(14):
            ...
            tcwv = float(dec72[x].tcwv[i,j])
            tcw = float(dec72[x].tcw[i,j])
            u10 = float(dec72[x].u10[i,j])
            v10 = float(dec72[x].v10[i,j])
            sst = float(dec72[x].sst[i,j])
            t2m = float(dec72[x].t2m[i,j])
            datasiconc = float(dec72[x].siconc[i,j])
            ...
            if lat > 50:
                ...
                fcomisotemp.append([siconccalc,lat,lon,TB22,TB31,tcwv,tcw,u10,v10,sst,t2m,datasiconc])
            elif lat < -50:
...
for x in range(len(jun73)):
    ...
    for i in range(len(jun73[x].TBNEMS)):
        for j in range(14):
            ...
            tcwv = float(jun73[x].tcwv[i,j])
            tcw = float(jun73[x].tcw[i,j])
            u10 = float(jun73[x].u10[i,j])
            v10 = float(jun73[x].v10[i,j])
            sst = float(jun73[x].sst[i,j])
            t2m = float(jun73[x].t2m[i,j])
            datasiconc = float(jun73[x].siconc[i,j])
            if lat > 50:
                af = (tpfy31n[x] - tpmy31n[x])/(tpfy22n[x] - tpmy22n[x])
                bf = (tpmy31n[x] - af*tpmy22n[x])
                qf = (TB31 - tpvand31n[x])/(TB22 - tpvand22n[x])
...
with open('RTMformatfcomisoDECEMBER.pkl', 'wb') as f:
    pickle.dump(combined_listdec, f)
with open('RTMformatfcomisoJUNI.pkl', 'wb') as f:
