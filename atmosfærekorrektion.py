# -*- coding: utf-8 -*-
"""
Created on Thu Jun  1 08:56:13 2023
...
for x in range(len(december)):
    TihogTivx = 0.6*272+0.4*float(december[x,10])
    TihogTiv = np.ones(7)*TihogTivx    
    SIC = float(december[x,0])
    V = december[x,5]
    W = np.sqrt((december[x,7])**2+(december[x,8])**2)
    L = december[x,6]-december[x,5]
    t2m = december[x,10]
    sst = december[x,9]
    lat = december[x,1]
    lon = december[x,2]
    era5sic = december[x,11]
    ...
    korT22 = avgT22-T22
    korT31 = avgT31-T31
    nyT31 = december[x,4]+korT31
    nyT22 = december[x,3]+korT22
    RTMDecember.append([nyT31,nyT22,lat,lon,sst,SIC,era5sic])
    print(f"progcheck dec :{x} er done")

RTMDecember = np.array(RTMDecember)    
with open('RTMkorrigeretDecember.pkl', 'wb') as f:
    pickle.dump(RTMDecember, f)
#%% RTM JUNI
RTMJuni = []

Vmean = float(np.nanmean(juni[:,5]))
Lmean = float(np.nanmean(juni[:,6]-juni[:,5]))
Wmean = float(np.nanmean(np.sqrt((juni[:,7])**2+(juni[:,8])**2)))
t2mmean = float(np.nanmean(juni[:,10]))
sstmean = float(np.nanmean(juni[:,9]))
theta=0
eicev = np.ones(7)*0.8
eiceh = np.ones(7)*0.8

for x in range(len(juni)):
    TihogTivx = 0.6*272+0.4*float(juni[x,10])
    TihogTiv = np.ones(7)*TihogTivx    
    SIC = float(juni[x,0])
    V = december[x,5]
    W = np.sqrt((juni[x,7])**2+(juni[x,8])**2)
    L = juni[x,6]-juni[x,5]
    t2m = juni[x,10]
    sst = juni[x,9]
    lat = juni[x,1]
    ...
    korT22 = avgT22-T22
    korT31 = avgT31-T31
    nyT31 = juni[x,4]+korT31
    nyT22 = juni[x,3]+korT22
    RTMJuni.append([nyT31,nyT22,lat,lon,sst,SIC,era5sic])
    print(f"progcheck jun :{x} er done")

RTMJuni = np.array(RTMJuni)
with open('RTMKorrigeretJuni.pkl', 'wb') as f:
    pickle.dump(RTMJuni, f)

#%% Brightness temperaturplot
colorsdec72 = []
for x in range(len(RTMDecember)):
   ...
   elif RTMDecember[x,5] > -5:
    colorsdec72.append('blue')
   else:
       colorsdec72.append('black')
sizesdec72 = [0.3 if c == 'blue' else 0.3 for c in colorsdec72]
...
for x in range(len(RTMJuni)):
   if RTMJuni[x,5] >= 0.9:
    ...
    colorsjun73.append('blue')
       ...
       colorsjun73.append('black')
sizesjun73 = [0.3 if c == 'blue' else 0.3 for c in colorsjun73]

plt.scatter(RTMDecember[:,0],RTMDecember[:,1],s=sizesdec72,c=colorsdec72)
plt.title("December 72 2-Chn Siconc")
plt.xlim([100, 330])
plt.ylim([100, 330])
...
plt.scatter(RTMJuni[:,0],RTMJuni[:,1],s=sizesjun73,c=colorsjun73)
plt.title("Juni 73 2-Chn Siconc")
plt.xlim([100, 330])
plt.ylim([100, 330])
plt.xlabel('TB 31.4 GHz [K]')
plt.ylabel('TB 22.235 GHz [K]')
